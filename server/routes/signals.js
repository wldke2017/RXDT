...
// Runs at exactly 5pm, 6pm, and 7pm EAT (14:00, 15:00, 16:00 UTC).
// Finds all users with available_balance >= 100 and automatically
// allocates 100% of their balance into the active signal trade,
// preventing duplicates for users who already executed manually.
export async function autoExecuteSignals() {
  const signal = await getActiveSignal();
  if (!signal) return { executed: 0, skipped: 0, message: 'No active signal window right now.' };

  const today = new Date().toISOString().split('T')[0];

  try {
    // Get all users with balance >= $100
    const usersRes = await query(
      `SELECT id, name, available_balance FROM users WHERE available_balance >= 100`
    );
    const users = usersRes.rows;
    if (!users.length) return { executed: 0, skipped: 0, message: 'No eligible users found.' };

    let executed = 0;
    let skipped = 0;

    for (const user of users) {
      // Skip users who already executed for this signal today (manual or auto)
      const execCheck = await query(
        `SELECT id FROM signal_trades WHERE user_id = $1 AND signal_id = $2 AND DATE(created_at) = $3`,
      ...
      const balance = parseFloat(user.available_balance);
      if (balance < 100) { skipped++; continue; }

      const tier = getTier(balance, false);
      ...
      const now = Date.now();
      const tradeId = 'ST' + now + user.id.slice(-4);
      const openId = 'AC' + now + 'A' + user.id.slice(-4);

      try {
        await query('BEGIN');

        // Move available_balance into frozen_balance
        await query(
          `UPDATE users 
           SET available_balance = available_balance - $1,
               frozen_balance = frozen_balance + $1
        ...
        await query(
          `INSERT INTO signal_trades (id, user_id, signal_id, pair, trade_amount, profit, balance_before, balance_after, tier_label, status, release_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'open', $10)`,
          [tradeId, user.id, signal.signalId, signal.pairSymbol, tradeAmount, profitAmount, balance, newBalance, tier.label, releaseAt]
        );
        ...
        await query(
          `INSERT INTO account_changes (id, user_id, type, amount, balance_after, remark)
           VALUES ($1, $2, 'signal_open', $3, 0, $4)`,
          [openId, user.id, -tradeAmount, `Signal ${signal.signalId} — Auto-executed (${signal.pairSymbol}) placed in Order`]
        );
        ...
        await query('ROLLBACK').catch(() => { });
        console.error(`Auto-execute error for user ${user.id}:`, err.message);
        skipped++;
      }
    }

    console.log(`✅ Auto-executed Signal ${signal.signalId}: ${executed} executed, ${skipped} skipped.`);
    return { executed, skipped, signalId: signal.signalId, message: `Signal ${signal.signalId} auto-executed for ${executed} users.` };
  } catch (err) {
    console.error('Auto-execute signals error:', err);
